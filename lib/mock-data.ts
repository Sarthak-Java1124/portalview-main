...
const SEED_JOBS: ReviewJob[] = [
  // ── Active / open jobs (reviewers can submit findings) ────────────────────
  {
    id: "job-crowdfunding",
    submitter: ADDR.r13,
    contractHash: "0x9af1c203e5b8d047f3c2a9e8d1f6b3c4e7a2d5f8b1e4a7c0d3f6a9c2e5b8d1f4",
    githubUrl: "https://github.com/use-ink/ink-examples/tree/main/fundraiser",
    description: "Milestone-based crowdfunding contract. Backers pledge PSP22 tokens; creator unlocks funds per approved milestone voted on by backers. Focus on backer refund path when milestone is rejected, reentrancy in claim(), and creator ability to drain before first milestone.",
    stakeAmount: 20n * POT,
    rewardPool: 20n * POT,
    status: "Open",
    openedAtBlock: 1_247_820,
    closesAtBlock: 1_249_020,
    findingCount: 0,
  },
  {
    id: "job-dao-treasury",
    submitter: ADDR.r14,
    contractHash: "0x8be0d192f4a7c036e2b1d9f8a5c4b3d6e8f0a2c4e6b8d0f2a4c6e8b0d2f4a6c8",
    githubUrl: "https://github.com/use-ink/ink-examples/tree/main/multi-sig",
    description: "DAO treasury with on-chain voting and a 48-hour timelock. Members vote to release funds to approved addresses. Review: timelock bypass via batch execution, quorum manipulation with flash-borrowed governance tokens, and treasury drain through repeated small-below-threshold proposals.",
    stakeAmount: 35n * POT,
    rewardPool: 35n * POT,
    status: "Open",
    openedAtBlock: 1_247_750,
    closesAtBlock: 1_248_950,
    findingCount: 0,
  },
  {
    id: "job-vesting-schedule",
    submitter: ADDR.r15,
    contractHash: "0x7cd9e081f3b6a025d1c0e8f7b4a3c2d5e7f9a1c3e5b7d9f1a3c5e7b9d1f3a5c7",
    githubUrl: "https://github.com/use-ink/ink-examples/tree/main/contract-transfer",
    description: "Token vesting contract with cliff + linear release schedule. Multiple beneficiaries, each with independent start time and cliff. Investigate: cliff bypass by setting start_time in the past, beneficiary substitution before cliff expiry, integer overflow in vested_amount() near u128::MAX, and admin revoke releasing tokens to wrong address.",
    stakeAmount: 18n * POT,
    rewardPool: 18n * POT,
    status: "InReview",
    openedAtBlock: 1_247_680,
    closesAtBlock: 1_248_880,
    findingCount: 2,
  },
  {
    id: "job-dex-amm-v2",
    ...
    githubUrl: "https://github.com/use-ink/ink-examples/tree/main/uniswap-v2",
  ...
  },
  {
    ...
    githubUrl: "https://github.com/use-ink/ink-examples/tree/main/upgradeable-contracts",
  ...
  },
  {
    ...
    githubUrl: "https://github.com/use-ink/ink-examples/tree/main/contract-terminate",
  ...
  // ── Completed ─────────────────────────────────────────────────────────────
  {
    ...
    githubUrl: "https://github.com/use-ink/ink-examples/tree/main/basic-contract-caller",
  ...
  },
  {
    ...
    githubUrl: "https://github.com/use-ink/ink-examples/tree/main/psp22-extension",
  ...
  },
  {
    ...
    githubUrl: "https://github.com/use-ink/ink-examples/tree/main/erc20",
  ...
  },
  {
    ...
    githubUrl: "https://github.com/use-ink/ink-examples/tree/main/rand-extension",
...
const SEED_FINDINGS: Finding[] = [
  ...
  // Vesting Schedule (InReview)
  { id: "find-vest-001", jobId: "job-vesting-schedule", reviewer: ADDR.alice, severity: "High", title: "Cliff bypass by setting start_time in the past", description: "create_schedule() accepts a start_time parameter without validating it is not in the past. An admin can immediately unlock the cliff portion for any beneficiary by setting start_time to block 0, bypassing the intended lock-up.", lineStart: 67, lineEnd: 74, submittedAtBlock: 1_247_700 },
  { id: "find-vest-002", jobId: "job-vesting-schedule", reviewer: ADDR.bob, severity: "Medium", title: "Overflow in vested_amount() when elapsed_blocks exceeds u64::MAX / total_tokens", description: "elapsed_blocks * total_tokens / duration_blocks is computed without checked arithmetic. For large total_tokens values and long durations, the intermediate multiplication silently wraps, returning a vested amount near zero and permanently locking funds.", lineStart: 112, lineEnd: 116, submittedAtBlock: 1_247_720 },
  // Stablecoin Vault (Consensus)
  { id: "find-vault-001", jobId: "job-stablecoin-vault", reviewer: ADDR.alice,   severity: "Critical",      title: "Flash loan oracle manipulation bypasses liquidation threshold",                    description: "get_collateral_ratio() reads spot price from a single oracle call with no TWAP. An attacker takes a flash loan, manipulates the oracle price 3× upward within the same block, borrows at an inflated collateral ratio, then lets the price revert — leaving an undercollateralised position that cannot be immediately liquidated.", lineStart: 198, lineEnd: 214, submittedAtBlock: 1_247_120 },
  { id: "find-vault-002", jobId: "job-stablecoin-vault", reviewer: ADDR.bob,     severity: "Critical",      title: "Liquidation bonus overflows reserve when collateral price drops 50%+",              description: "liquidate() calculates collateral_seized = debt_repaid * (1 + bonus_pct) / collateral_price. When collateral_price is very low, collateral_seized exceeds total vault collateral, causing the reserve balance to underflow and wrap to u128::MAX, permanently corrupting accounting.", lineStart: 267, lineEnd: 285, submittedAtBlock: 1_247_140 },
  { id: "find-vault-003", jobId: "job-stablecoin-vault", reviewer: ADDR.charlie, severity: "High",          title: "Collateral ratio rounding always truncates in borrower's favour",                   description: "collateral_ratio() computes (collateral_value * 100) / debt using integer division, truncating the remainder. A borrower can maintain a position that is mathematically undercollateralised by up to 0.99% without triggering the liquidation check.", lineStart: 143, lineEnd: 147, submittedAtBlock: 1_247_155 },
  { id: "find-vault-004", jobId: "job-stablecoin-vault", reviewer: ADDR.dave,    severity: "High",          title: "No liquidation incentive for dust positions creates permanent bad debt",           description: "For vault positions below min_liquidation_size, the liquidation bonus is smaller than gas cost. No liquidator will touch dust positions, which accumulate as perpetual bad debt draining the stability reserve over time.", lineStart: 302, lineEnd: 310, submittedAtBlock: 1_247_170 },
  { id: "find-vault-005", jobId: "job-stablecoin-vault", reviewer: ADDR.eve,     severity: "Medium",        title: "Emergency pause callable by any address with OPERATOR role",                      description: "set_paused() requires only the OPERATOR role, which is granted by the deployer to a keeper bot address. If the keeper private key is compromised, an attacker can permanently pause all vault operations — blocking withdrawals — without any additional multisig requirement.", lineStart: 55, lineEnd: 60, submittedAtBlock: 1_247_185 },
  { id: "find-vault-006", jobId: "job-stablecoin-vault", reviewer: ADDR.ferdie,  severity: "Low",           title: "Stale oracle price accepted up to 60 minutes after last update",                   description: "The staleness check allows prices up to 3600 blocks old (≈60 min on Portaldot). During high volatility this window is too wide — a 60-minute-old price can diverge >15% from current market, enabling profitable borrows against stale collateral valuations.", lineStart: 88, lineEnd: 91, submittedAtBlock: 1_247_195 },
  { id: "find-vault-007", jobId: "job-stablecoin-vault", reviewer: ADDR.r7,      severity: "Informational", title: "get_all_vaults() iterates unbounded storage mapping",                              description: "get_all_vaults() collects every vault entry into a Vec before returning. With thousands of open vaults this call will exceed block gas limits, making the view function unusable from on-chain callers and unreliable for off-chain indexers.", submittedAtBlock: 1_247_205 },
...
const SEED_CONSENSUS: Record<string, ConsensusState> = {
  "job-crowdfunding":      { jobId: "job-crowdfunding",      totalFindings: 0, confirmedFindings: 0, threshold: 2, reached: false, votingEnds: 1_249_020 },
  "job-dao-treasury":      { jobId: "job-dao-treasury",      totalFindings: 0, confirmedFindings: 0, threshold: 2, reached: false, votingEnds: 1_248_950 },
  "job-vesting-schedule":  { jobId: "job-vesting-schedule",  totalFindings: 2, confirmedFindings: 1, threshold: 2, reached: false, votingEnds: 1_248_880 },
...
const SEED_ESCROW: Record<string, EscrowState> = {
  "job-crowdfunding":      { jobId: "job-crowdfunding",      submitter: ADDR.r13,    amount: 20n  * POT, status: "Staked",    openedAtBlock: 1_247_820 },
  "job-dao-treasury":      { jobId: "job-dao-treasury",      submitter: ADDR.r14,    amount: 35n  * POT, status: "Staked",    openedAtBlock: 1_247_750 },
  "job-vesting-schedule":  { jobId: "job-vesting-schedule",  submitter: ADDR.r15,    amount: 18n  * POT, status: "Staked",    openedAtBlock: 1_247_680 },
